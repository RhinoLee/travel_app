...
<template>
  <div v-if="placeDetail" :class="{ hidden: !isPanelOpen }" class="w-[300px] md:w-[358px] h-[calc(94vh-54px)]
      absolute left-1/2 md:left-[308px] top-[50%] -translate-y-[50%] -translate-x-[50%] md:-translate-x-0 py-[14px] px-[20px] 
    ...
    <div class="flex flex-col h-[calc(100%-72px)]">
      ...
      <!-- 景點圖片 -->
      <div class="mb-[12px] w-full overflow-hidden rounded-[10px] ">
        <img class="w-full h-auto" v-if="placeDetail.photos && placeDetail.photos.length > 0"
      ...
      <div class="child:py-[4px] md:child:py-[8px]">
        ...
        <div class="py-[8px]">
          <p class="pb-1">營業時間：</p>
          <div v-if="placeDetail.opening_hours" class="max-h-[70px] md:max-h-[200px] overflow-y-scroll no-scrollbar">
    ...
    <!-- 按鈕區 -->
    <div class="absolute bottom-0 left-0 flex items-center border-t border-travel-green px-[20px] w-full h-[72px]">
